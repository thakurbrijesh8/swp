...
class Msme extends CI_Controller {
    ...
    function get_msme_data() {
        ...
        $this->db->trans_start();
        $success_array['msme_data'] = $this->utility_model->get_result_data_by_id('user_id', $session_user_id, 'incentive_generalform', NULL, NULL, 'incentive_id', 'DESC');
    ...
    function submit_msme() {
        ...
        if ($module_type == VALUE_TWO) {
            $this->utility_lib->send_sms_and_email_for_app_submitted($user_id, VALUE_SIX, VALUE_NINE, $incentive_id);
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
            echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
    ...
    function get_msme_data_by_incentive_id() {
        $session_user_id = get_from_session('temp_id_for_eodbsws');
        if (!is_post() || $session_user_id == NULL || !$session_user_id) {
            echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
            return false;
    ...
    function remove_fees_paid_challan() {
        ...
        $file_path = 'documents' . DIRECTORY_SEPARATOR . 'incentive_generalform' . DIRECTORY_SEPARATOR . $ex_est_data['fees_paid_challan'];
        if (file_exists($file_path)) {
            unlink($file_path);
        }
        $this->utility_model->update_data('incentive_id', $incentive_id, 'incentive_generalform', array('status' => VALUE_THREE, 'fees_paid_challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
        $success_array = get_success_array();
        $success_array['message'] = DOCUMENT_REMOVED_MESSAGE;
        echo json_encode($success_array);
    }
    ...
    function upload_fees_paid_challan() {
        ...
        if ($_FILES['fees_paid_challan_for_msme_upload_challan']['name'] != '') {
            ...
            $module_path = $documents_path . DIRECTORY_SEPARATOR . 'wmmsme';
            if (!is_dir($module_path)) {
                mkdir($module_path);
                chmod($module_path, 0777);
            }
