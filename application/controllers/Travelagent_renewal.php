...
class Travelagent_renewal extends CI_Controller {
    ...
    function get_travelagent_renewal_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['travelagent_renewal_data'] = $this->utility_model->get_result_data_by_id('user_id', $session_user_id, 'travelagent_renewal', 'area_of_agency', $search_district, 'travelagent_renewal_id', 'DESC', 'status', $search_status);
    ...
    function get_travelagent_data_by_id() {
        try {
            ...
            $this->db->trans_start();
            $travelagent_data = $this->utility_model->get_by_id('registration_number', $license_number, 'travelagent');
    ...
    function submit_travelagent_renewal() {
        try {
            ...
            $travelagent_renewal_data['status'] = $module_type;
            if ($module_type == VALUE_TWO) {
                $travelagent_renewal_data['submitted_datetime'] = date('Y-m-d H:i:s');
            }
            if (!$travelagent_renewal_id || $travelagent_renewal_id == NULL) {
                $travelagent_renewal_data['user_id'] = $user_id;
            ...
            if ($module_type == VALUE_TWO) {
                $this->utility_lib->send_sms_and_email_for_app_submitted($user_id, VALUE_SIX, VALUE_TWENTYTHREE, $travelagent_renewal_id);
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
    ...
    function get_travelagent_renewal_data_by_travelagent_renewal_id() {
        try {
            ...
            $this->payment_lib->get_payment_history_data($session_user_id, VALUE_TWENTYTHREE, $travelagent_renewal_id, $travelagent_renewal_data);
            $travelagent_renewal_data['fb_data'] = $this->utility_model->get_result_data_by_id('module_type', VALUE_TWENTYTHREE, 'fees_bifurcation', 'module_id', $travelagent_renewal_id);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
                return;
    ...
    function remove_fees_paid_challan() {
        try {
            ...
            $this->utility_model->update_data('travelagent_renewal_id', $travelagent_renewal_id, 'travelagent_renewal', array('status' => VALUE_THREE, 'fees_paid_challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
            $success_array['message'] = DOCUMENT_REMOVED_MESSAGE;
            echo json_encode($success_array);
        } catch (\Exception $e) {
    ...
    function upload_fees_paid_challan() {
        try {
            ...
            $ex_em_data = $this->utility_model->get_by_id('travelagent_renewal_id', $travelagent_renewal_id, 'travelagent_renewal');
            if (empty($ex_em_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            ...
            if ($ex_em_data['payment_type'] == VALUE_TWO) {
                $user_payment_type = get_from_post('user_payment_type_for_travelagent_renewal_upload_challan');
            ...
            $travelagent_renewal_data = array();
            if ($_FILES['fees_paid_challan_for_travelagent_renewal_upload_challan']['name'] != '') {
                ...
                $temp_filename = str_replace('_', '', $_FILES['fees_paid_challan_for_travelagent_renewal_upload_challan']['name']);
                ...
                if (!move_uploaded_file($_FILES['fees_paid_challan_for_travelagent_renewal_upload_challan']['tmp_name'], $final_path)) {
                    echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                    return;
                }
                $travelagent_renewal_data['status'] = VALUE_FOUR;
                $travelagent_renewal_data['fees_paid_challan'] = $filename;
                $travelagent_renewal_data['fees_paid_challan_updated_date'] = date('Y-m-d H:i:s');
            }
            if ($ex_em_data['payment_type'] == VALUE_TWO) {
                if ($user_payment_type == VALUE_TWO) {
                    $travelagent_renewal_data['status'] = VALUE_FOUR;
                    $travelagent_renewal_data['status'] = VALUE_EIGHT;
                } else if ($user_payment_type == VALUE_THREE) {
                    $travelagent_renewal_data['status'] = VALUE_THREE;
                    $enc_pg_data = $this->payment_lib->get_encrypted_details_for_pg($user_id, VALUE_TWENTYTHREE, $travelagent_renewal_id, $ex_em_data['area_of_agency'], $ex_em_data['total_fees'], $travelagent_renewal_data);
                    if ($enc_pg_data['success'] == false) {
                        echo json_encode(get_error_array($enc_pg_data['message']));
                        return;
                    }
                }
                $travelagent_renewal_data['user_payment_type'] = $user_payment_type;
            } else {
                $travelagent_renewal_data['user_payment_type'] = VALUE_ZERO;
            }
            $travelagent_renewal_data['updated_by'] = $user_id;
            $travelagent_renewal_data['updated_time'] = date('Y-m-d H:i:s');
            $this->db->trans_start();
            ...
            $success_array = get_success_array();
            $success_array['status'] = isset($travelagent_renewal_data['status']) ? $travelagent_renewal_data['status'] : $ex_em_data['status'];
            $success_array['message'] = CHALLAN_UPLOADED_MESSAGE;
            $success_array['payment_type'] = $ex_em_data['payment_type'];
            $success_array['user_payment_type'] = $travelagent_renewal_data['user_payment_type'];
            if ($ex_em_data['payment_type'] == VALUE_TWO && $travelagent_renewal_data['user_payment_type'] == VALUE_THREE) {
    ...
    function generate_certificate() {
        try {
            ...
            error_reporting(E_ERROR);
            $this->utility_lib->gc_for_travelagent_renewal($existing_travelagent_renewal_data);
        } catch (\Exception $e) {
            print_r($e->getMessage());
            return false;
        }
    ...
    function upload_travelagentrenewal_document() {
        try {
            if (!is_ajax()) {
                header("Location:" . base_url() . "login");
                return false;
            ...
            if ($file_no == VALUE_ONE) {
                $travelagent_renewal_data = $this->utility_model->upload_document('seal_and_stamp_for_travelagentrenewal', 'travelagent', 'signatur_', 'signature');
            }
            if (!$travelagent_renewal_data) {
                return false;
            }
            $this->db->trans_start();
            if (!$travelagent_renewal_id) {
                $travelagent_renewal_data['user_id'] = $session_user_id;
                $travelagent_renewal_data['status'] = VALUE_ONE;
                $travelagent_renewal_data['created_by'] = $session_user_id;
                $travelagent_renewal_data['created_time'] = date('Y-m-d H:i:s');
                $travelagent_renewal_id = $this->utility_model->insert_data('travelagent_renewal', $travelagent_renewal_data);
            } else {
                $travelagent_renewal_data['updated_by'] = $session_user_id;
                $travelagent_renewal_data['updated_time'] = date('Y-m-d H:i:s');
                $this->utility_model->update_data('travelagent_renewal_id', $travelagent_renewal_id, 'travelagent_renewal', $travelagent_renewal_data);
            }
            ...
            $success_array = get_success_array();
            $success_array['travelagent_renewal_data'] = $travelagent_renewal_data;
            $success_array['travelagent_renewal_id'] = $travelagent_renewal_id;
