...
SingleReturn.Router = Backbone.Router.extend({
    ...
    // renderListForPartADetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartADetails(encryptId);
    // },
    // renderListForPartBDetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartBDetails(encryptId);
    // },
    // renderListForPartCDetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartCDetails(encryptId);
    // },
    // renderListForPartDDetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartDDetails(encryptId);
    // },
    // renderListForPartEDetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartEDetails(encryptId);
    // },
    // renderListForPartfDetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartfDetails(encryptId);
    // },
    // renderListForPartGDetails: function (encryptId) {
    //     SingleReturn.listview.listPageForPartGDetails(encryptId);
    // },
});
SingleReturn.listView = Backbone.View.extend({
    ...
    listPageForSingleReturn: function (sDistrict, sStatus) {
        this.loadSingleReturn(sDistrict, sStatus);
        this.loadSingleReturnData(sDistrict, sStatus);
    },
    loadSingleReturn: function (sDistrict, sStatus) {
    ...
    //     addClass('menu_single_return', 'active');
    //     SingleReturn.router.navigate('singlereturn');
    //     var templateData = {};
    //     this.$el.html(singleReturnListTemplate(templateData));
    //     this.loadSingleReturnData();
    ...
    // listPageForPartADetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editOrViewPartA($('#submit_btn_for_partb_details'), encryptId);
    // },
    // listPageForPartBDetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editRoadAccidentDetails($('#submit_btn_for_accident'), encryptId);
    // },
    // listPageForParCDetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editRoadAccidentDetails($('#submit_btn_for_accident'), encryptId);
    // },
    // listPageForParDDetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editRoadAccidentDetails($('#submit_btn_for_accident'), encryptId);
    // },
    // listPageForParEDetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editRoadAccidentDetails($('#submit_btn_for_accident'), encryptId);
    // },
    // listPageForParFDetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editRoadAccidentDetails($('#submit_btn_for_accident'), encryptId);
    // },
    // listPageForParGDetails: function (encryptId) {
    //     this.loadSingleReturn();
    //     //this.editRoadAccidentDetails($('#submit_btn_for_accident'), encryptId);
    // },
    actionRenderer: function (rowData) {
        ...
        if (rowData.status != VALUE_ZERO && rowData.status != VALUE_ONE && rowData.status != VALUE_TWO && rowData.status != VALUE_SIX && rowData.status != VALUE_NINE) {
            if (rowData.payment_type != VALUE_THREE && rowData.payment_type != VALUE_ZERO) {
                rowData.ADMIN_SINGLERETURN_DOC_PATH = ADMIN_SINGLERETURN_DOC_PATH;
    ...
    loadSingleReturnData: function (sDistrict, sStatus) {
        ...
        singleReturnDataTable = $('#single_return_datatable').DataTable({
            ajax: {url: 'singlereturn/get_single_return_data', dataSrc: "single_return_data", type: "post", data: $.extend({}, searchData, getTokenData())},
            bAutoWidth: false,
            pageLength: 10,
            ordering: false,
            ...
            columns: [
                ...
                {data: 'singlereturn_id', 'class': 'v-a-m text-center f-w-b', 'render': tempRegNoRenderer},
                {data: 'district', 'class': 'text-center', 'render': districtRenderer},
                {data: 'entity_establishment_type', 'class': 'text-center', 'render': entityEstablishmentRenderer},
                {data: 'esta_name', 'class': 'text-center'},
                {data: 'esta_address', 'class': 'text-center'},
                {data: 'esta_email_id', 'class': 'text-center'},
                {data: 'submitted_datetime', 'class': 'text-center', 'render': dateTimeRenderer},
                {data: 'singlereturn_id', 'class': 'v-a-m text-center', 'render': newAppStatusRenderer},
                {data: 'singlereturn_id', 'class': 'v-a-m text-center', 'render': queryStatusRenderer},
    ...
    checkValidationForSingleReturn: function (singleReturnData) {
        ...
        if (!singleReturnData.entity_establishment_type) {
    ...
    submitSingleReturn: function (moduleType) {
        ...
        singleReturnData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartADetails: function (moduleType) {
        ...
        singleReturnPartAData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartBDetails: function (moduleType) {
        ...
        singleReturnPartBData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartCDetails: function (moduleType) {
        ...
        singleReturnPartCData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartDDetails: function (moduleType) {
        ...
        // console.log(dangerousProcessInfoItem);
        // console.log(hazardousProcessInfoItem);
        //var btnObj = moduleType == VALUE_ONE ? $('#submit_btn_for_single_return') : $('#submit_btn_for_single_return');
        ...
        singleReturnPartDData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartEDetails: function (moduleType) {
        ...
        singleReturnPartEData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartFDetails: function (moduleType) {
        ...
        singleReturnPartFData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    submitPartGDetails: function (moduleType) {
        ...
        singleReturnPartGData.append("csrf_token_eodbsws", getTokenData()['csrf_token_eodbsws']);
    ...
    downloadUploadChallan: function (singlereturnId) {
        if (!singlereturnId) {
            showError(invalidAccessValidationMessage);
            return false;
        }
        var that = this;
        var btnObj = $('#download_upload_btn_' + singlereturnId);
        var ogBtnHTML = btnObj.html();
        var ogBtnOnclick = btnObj.attr('onclick');
        btnObj.html(iconSpinnerTemplate);
        btnObj.attr('onclick', '');
        $.ajax({
            ...
            success: function (response) {
                ...
                var singleReturnData = parseData.single_return_data;
                that.showChallan(singleReturnData);
            }
        });
    },
    showChallan: function (singleReturnData) {
        showPopup();
        if (singleReturnData.status != VALUE_FIVE && singleReturnData.status != VALUE_SIX && singleReturnData.status != VALUE_SEVEN  && singleReturnData.status != VALUE_ELEVEN) {
            if (!singleReturnData.hide_submit_btn) {
                singleReturnData.show_fees_paid = true;
            }
        }
        if (singleReturnData.payment_type == VALUE_ONE) {
            singleReturnData.utitle = 'Fees Paid Challan Copy';
        } else {
            singleReturnData.style = 'display: none;';
        }
        if (singleReturnData.payment_type == VALUE_TWO) {
            singleReturnData.show_dd_po_option = true;
            singleReturnData.utitle = 'Demand Draft (DD)';
        }
        singleReturnData.module_type = VALUE_THIRTYNINE;
        $('#popup_container').html(singleReturnUploadChallanTemplate(singleReturnData));
        loadFB(VALUE_THIRTYNINE, singleReturnData.fb_data);
        loadPH(VALUE_THIRTYNINE, singleReturnData.singlereturn_id, singleReturnData.ph_data);

        if (singleReturnData.payment_type == VALUE_TWO) {
            generateBoxes('radio', userPaymentTypeArray, 'user_payment_type', 'single_return_upload_challan', singleReturnData.user_payment_type, true);
            showSubContainerForPaymentDetails('user_payment_type', 'single_return_upload_challan', 'uc', 'radio');
            if (singleReturnData.user_payment_type == VALUE_ZERO) {
                $('input[name=user_payment_type_for_single_return_upload_challan][value="' + VALUE_ONE + '"]').click();
            }
        }
        if (singleReturnData.challan != '') {
            ...
            $('#challan_name_href_for_single_return_upload_challan').attr('href', ADMIN_SINGLERETURN_DOC_PATH + singleReturnData.challan);
            $('#challan_name_for_single_return_upload_challan').html(singleReturnData.challan);
        }
        if (singleReturnData.fees_paid_challan != '') {
            $('#fees_paid_challan_container_for_single_return_upload_challan').hide();
            $('#fees_paid_challan_name_container_for_single_return_upload_challan').show();
            $('#fees_paid_challan_name_href_for_single_return_upload_challan').attr('href', 'documents/singlereturn/' + singleReturnData.fees_paid_challan);
            $('#fees_paid_challan_name_for_single_return_upload_challan').html(singleReturnData.fees_paid_challan);
            $('#fees_paid_challan_remove_btn_for_single_return_upload_challan').attr('onclick', 'SingleReturn.listview.removeFeesPaidChallan("' + singleReturnData.singlereturn_id + '")');
        }
    },
    removeFeesPaidChallan: function (singlereturnId) {
        if (!tempIdInSession || tempIdInSession == null) {
            loginPage();
            return false;
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'singlereturn/remove_fees_paid_challan',
            data: $.extend({}, {'singlereturn_id': singlereturnId}, getTokenData()),
            error: function (textStatus, errorThrown) {
                generateNewCSRFToken();
                if (textStatus.status === 403) {
            ...
            success: function (response) {
                ...
                removeDocument('fees_paid_challan', 'single_return_upload_challan');
                $('#status_' + singlereturnId).html(appStatusArray[VALUE_THREE]);
            }
        });
    },
    uploadFeesPaidChallan: function () {
        ...
        validationMessageHide();
        var singlereturnId = $('#single_return_id_for_single_return_upload_challan').val();
        if (!singlereturnId) {
            showError(invalidAccessValidationMessage);
            return false;
        }
        if ($('#fees_paid_challan_container_for_single_return_upload_challan').is(':visible')) {
            var sealAndStamp = $('#fees_paid_challan_for_single_return_upload_challan').val();
            if (sealAndStamp == '') {
                $('#fees_paid_challan_for_single_return_upload_challan').focus();
                validationMessageShow('single-return-uc-fees_paid_challan_for_single_return_upload_challan', uploadDocumentValidationMessage);
                return false;
            }
            var challanMessage = fileUploadValidation('fees_paid_challan_for_single_return_upload_challan', 2048);
            if (challanMessage != '') {
                $('#fees_paid_challan_for_single_return_upload_challan').focus();
                validationMessageShow('single-return-uc-fees_paid_challan_for_single_return_upload_challan', challanMessage);
                return false;
            }
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'singlereturn/upload_fees_paid_challan',
            data: formData,
            mimeType: "multipart/form-data",
            contentType: false,
            cache: false,
            ...
            success: function (data) {
                ...
                Swal.close();
                $('#status_' + singlereturnId).html(appStatusArray[parseData.status]);
