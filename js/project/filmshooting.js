...
FilmShooting.listView = Backbone.View.extend({
    ...
    loadFilmShootingData: function (sDistrict, sStatus) {
        ...
        var tempRegNoRenderer = function (data, type, full, meta) {
            return regNoRenderer(VALUE_TWENTYTWO, data)
                    + getFRContainer(VALUE_TWENTYTWO, data, full.rating, full.fr_datetime);
        };
        var that = this;
        FilmShooting.router.navigate('filmshooting');
    ...
    showChallan: function (filmShootingData) {
        showPopup();
        if (filmShootingData.status != VALUE_FIVE && filmShootingData.status != VALUE_SIX && filmShootingData.status != VALUE_SEVEN && filmShootingData.status != VALUE_ELEVEN) {
